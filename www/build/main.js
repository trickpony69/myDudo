...
var ProfileProvider = /** @class */ (function () {
    ...
    ProfileProvider.prototype.getFriendForAList = function (list) {
        var _this = this;
        return new Promise(function (resolve) {
            var arr = [];
            var friendsLists = __WEBPACK_IMPORTED_MODULE_1_firebase_app___default.a.database().ref('/todos/' + _this.userProfile.key + '/' + list + '/' + '/friends/');
    ...
    ProfileProvider.prototype.setFriends = function (friendId, list, i, path) {
        __WEBPACK_IMPORTED_MODULE_1_firebase_app___default.a.database().ref('userProfile/' + friendId + '/sharedLists' + '/list' + i).update({
            n: i,
            title: list,
            path: path
        });
        __WEBPACK_IMPORTED_MODULE_1_firebase_app___default.a.database().ref('todos/' + this.userProfile.key + '/' + list + '/' + 'friends/' + friendId).set({
            name: 'utente amico'
        });
    };
...
var HomeListe = /** @class */ (function () {
    ...
    HomeListe.prototype.ionViewWillEnter = function () {
        var _this = this;
        this.profileProv.getFriendForAList("matteo").then(function (data) {
        });
        ...
        this.profileProv.getFriendLists().then(function (data) {
            ...
            data.forEach(function (element0) {
                ...
                if (!trovato) {
                    _this.sharedCards.push({ id: '0', name: element0.title, friends: "null", proprietary: 0, path: element0.path });
                    _this.cardCount++;
                }
            });
        });
    ...
    HomeListe.prototype.checkAlreadyAdded = function (friend, list) {
    ...
    HomeListe.prototype.addFriend = function (i) {
        var _this = this;
        var friends = this.profileProv.getPeople().then(function (data) {
            ...
            data.forEach(function (element, index) {
                alert.addInput({
                    ...
                    value: element.key,
                    checked: false,
                    disabled: _this.checkAlreadyAdded(element, element.key),
                });
            });
            ...
            alert.addButton({
                ...
                handler: function (friend) {
                    ...
                    _this.profileProv.setFriends(friend, _this.cards[i].name, i, path);
                }
            });
            alert.present();
        });
    ...
    HomeListe.prototype.presentActionSheet = function (card, index) {
        ...
        var popup = this.actionSheet.create({
            ...
            buttons: [
                {
                    text: "Aggiungi amico",
                    handler: function () { _this.addFriend(index); }
                }, {
                    text: "modifica",
